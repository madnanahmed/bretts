...
<div class="content-wrapper">
	<ol class="breadcrumb m-b-0">
		...
		<li class="active">Inactivated Properties</li>
	</ol>
	<div class="card table-responsive">
		<div class="card-header">
			<h5 class="card-title active">Inactive Properties <a class="btn btn-info" href="<?= base_url('admin/Property'); ?>">Back to property</a></h5>
		</div>
		<?php
		...
		<table id="property" class="table table-striped table-hover table-sm">
			...
			foreach($data as $value){?>
				<tr>
					<td><?= $value->id ?></td>
					<td><a href="#"><?= ucfirst($value->title) ?></a></td>
					<td>
						<ul>
					...
					<td>
						<?= ($value->status == 1)? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>' ?>
					</td>
					<td>
						<a class="btn btn-xs btn-danger" onclick="return confirm('Are you sure to delete property permanently? ')" href="<?= base_url('admin/delete_property_permanently/'.$value->id.'') ?>"> Delete</a>
					</td>
